...
async function sendWeekReminders() {
  try {
    ...
    for (const appointment of appointments) {
      ...
      const serviceList = appointmentData.services?.map((s: any) => s.name) || [];
      const staffName = appointmentData.staff?.user?.fullName;
      const appointmentDateStr = new Date(appointment.appointmentDate).toLocaleDateString();
      ...
      await EmailService.sendAppointmentReminder({
        ...
        businessName: appointmentData.business.businessName,
        appointmentDate: appointmentDateStr,
        startTime: appointment.startTime,
        endTime: appointment.endTime,
        services: serviceList,
        staffName,
        address: appointmentData.business.address,
      });
